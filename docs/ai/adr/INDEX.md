...
| [004](004-mvp-scope.md) | MVP Scope — Day-Trade Decision Dashboard | Proposed | Phase 1 = single-primary-instrument realtime recommender (ENTER/HOLD/EXIT/RETREAT). Setup engine + rule overlay + macro overlay + user-initiated AI chat (floating FAB). Three routes: `/login`, `/` dashboard (single primary + right-column Watchlist / NewsFeed widgets), `/settings`. Four providers (MarketData / EventCalendar / News / Chat). Only config is persisted — ticks/recommendations/news/P&L in-memory. Operator values in DB, secrets in `.env` |
