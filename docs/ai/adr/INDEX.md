...
| [003](003-ui-foundations.md) | UI Foundations | Proposed | shadcn/ui + Tailwind v4 + react-hook-form + zod + lucide-react + sonner + lightweight-charts; AppShell, dark mode default, 404/ErrorBoundary, jsx-a11y |
| [004](004-mvp-scope.md) | MVP Scope — Day-Trade Decision Cockpit | Proposed | Phase 1 = single-asset-class realtime recommender (ENTER/HOLD/EXIT/RETREAT). Setup engine + rule overlay + macro overlay + user-initiated AI chat. Two routes: `/` cockpit (chart-centric with annotations) and `/settings` (provider/threshold configuration). Operator values in DB, secrets in `.env`; no hardcoded personal values |
